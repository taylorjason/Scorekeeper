import{O as e,_ as t,f as n,g as r,j as i,p as a}from"./index-DSpfXjW8.js";var o=class{players=[];games=[];matches=[];async load(){let[t,n]=await Promise.all([i(),e()]);this.players=t.filter(e=>e.active),this.games=n}escHtml(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}render(){let e=new Date().toISOString().split(`T`)[0],t=this.games.length===0?`<option value="">No games — add in Settings</option>`:this.games.map(e=>`<option value="${e.id}">${this.escHtml(e.name)}</option>`).join(``),n=this.players.length===0?`<p class="text-sm text-muted">No players — add in Settings</p>`:this.players.map(e=>`
    ...
    `}renderMatchList(){return this.matches.length===0?`<p class="text-sm text-muted" id="no-matches-msg">No matches added yet. Add at least one above.</p>`:this.matches.map((e,t)=>`
    ...
    `).join(``)}afterRender(){document.getElementById(`back-btn`)?.addEventListener(`click`,()=>{a(`dashboard`)}),document.getElementById(`add-match-btn`)?.addEventListener(`click`,()=>{this.handleAddMatch()}),document.getElementById(`new-night-form`)?.addEventListener(`submit`,e=>{e.preventDefault(),this.handleSubmit()}),this.bindRemoveButtons()}bindRemoveButtons(){document.querySelectorAll(`.remove-match-btn`).forEach(e=>{e.addEventListener(`click`,e=>{let t=parseInt(e.currentTarget.dataset.index??`0`,10);this.matches.splice(t,1),this.refreshMatchList()})})}handleAddMatch(){let e=document.getElementById(`match-game`),t=parseInt(e.value,10);if(!t){n(`Please select a game`,`error`);return}let r=document.querySelectorAll(`.match-player-check:checked`),i=Array.from(r).map(e=>parseInt(e.value,10));if(i.length<1){n(`Select at least one player`,`error`);return}let a=this.games.find(e=>e.id===t);if(!a)return;let o=i.map(e=>this.players.find(t=>t.id===e)?.displayName??`Player ${e}`);this.matches.push({gameId:t,gameName:a.name,playerIds:i,playerNames:o}),this.refreshMatchList(),e.value=``,document.querySelectorAll(`.match-player-check`).forEach(e=>{e.checked=!1}),n(`Added ${a.name} to the list`,`success`)}refreshMatchList(){let e=document.getElementById(`match-list`);e&&(e.innerHTML=this.renderMatchList(),this.bindRemoveButtons());let t=document.getElementById(`match-count`);t&&(t.textContent=String(this.matches.length));let n=document.getElementById(`start-night-btn`);n&&(n.disabled=this.matches.length===0)}async handleSubmit(){let e=document.getElementById(`night-title`),i=document.getElementById(`night-date`),o=document.getElementById(`night-notes`),s=document.getElementById(`title-error`),c=e.value.trim();if(!c){e.classList.add(`error`),s&&(s.textContent=`Title is required`),e.focus();return}if(s&&(s.textContent=``),e.classList.remove(`error`),this.matches.length===0){n(`Add at least one match to the night`,`error`);return}let l=document.getElementById(`start-night-btn`);l.disabled=!0,l.textContent=`Creating...`;try{let e=await r({title:c,date:i.value||new Date().toISOString().split(`T`)[0],notes:o.value.trim()||void 0,createdAt:Date.now()}),s=[];for(let n of this.matches){let r=await t({gameNightId:e,gameId:n.gameId,playerIds:n.playerIds,status:`active`,createdAt:Date.now()});s.push(r)}n(`Game night created!`,`success`),a(`match`,{id:String(s[0])})}catch(e){console.error(`Failed to create game night:`,e),n(`Failed to create game night`,`error`),l.disabled=!1,l.textContent=`🎮 Start Game Night`}}};export{o as NewNight};