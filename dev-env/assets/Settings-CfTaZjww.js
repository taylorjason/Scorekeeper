...
`);return`
          ...
          <div class="cfield-add-row">
            <div class="icon-picker-wrap" style="position:relative">
              ...
              <div id="icon-picker-grid" class="icon-picker-grid" style="${this.iconPickerOpen?``:`display:none`}">
                ...
                ${K.map(e=>`<button type="button" class="emoji-opt${this.pendingFieldIcon===e?` emoji-opt--selected`:``}" data-emoji="${e}">${e}</button>`).join(``)}
              </div>
            </div>
    ...
    `}renderCustomFieldsList(){return this.pendingCustomFields.length===0?`<p class="text-xs text-muted" style="margin:0.25rem 0 0.5rem">No custom fields yet.</p>`:this.pendingCustomFields.map((e,t)=>`
      <div class="cfield-row">
        ${e.icon?`<span class="cfield-row-icon">${e.icon}</span>`:``}
