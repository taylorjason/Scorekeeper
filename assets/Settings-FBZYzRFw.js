import{C as e,F as t,L as n,N as r,O as i,a,b as o,c as s,d as c,f as l,h as u,i as d,j as f,l as p,n as m,o as h,r as g,s as _,t as v,u as y,v as b,w as x,y as S}from"./index-DFtt9rQR.js";import{n as C}from"./utils-BZltm_RL.js";var w=Object.create,T=Object.defineProperty,E=Object.getOwnPropertyDescriptor,D=Object.getOwnPropertyNames,O=Object.getPrototypeOf,k=Object.prototype.hasOwnProperty,A=(e,t)=>()=>(t||(e((t={exports:{}}).exports,t),e=null),t.exports),j=(e,t,n,r)=>{if(t&&typeof t==`object`||typeof t==`function`)for(var i=D(t),a=0,o=i.length,s;a<o;a++)s=i[a],!k.call(e,s)&&s!==n&&T(e,s,{get:(e=>t[e]).bind(null,s),enumerable:!(r=E(t,s))||r.enumerable});return e},M=(e,t,n)=>(n=e==null?{}:w(O(e)),j(t||!e||!e.__esModule?T(n,`default`,{value:e,enumerable:!0}):n,e)),N=[`#ef4444`,`#f97316`,`#f59e0b`,`#eab308`,`#10b981`,`#14b8a6`,`#3b82f6`,`#6366f1`,`#8b5cf6`,`#ec4899`,`#64748b`,`#d97706`];function P(e){return e.players.some(e=>e.round_scores.some(e=>e.phase!==void 0))?`phase10`:`rounds`}function F(e){let t=new Map;for(let n of e.players)for(let e of n.round_scores)t.has(e.round_number)||t.set(e.round_number,e.round_name);let n=[...t.entries()].sort((e,t)=>e[0]-t[0]);if(!n.every(([e,t])=>t===String(e)))return n.map(([,e])=>e)}function I(e,t,n){let r=e.players.filter(e=>e.is_winner);if(r.length===0)return;if(r.length===1)return t.get(r[0].name.trim().toLowerCase());let i=n===`phase10`||n===`low`?r.reduce((e,t)=>e.final_score<t.final_score?e:t):r.reduce((e,t)=>e.final_score>t.final_score?e:t);return t.get(i.name.trim().toLowerCase())}async function L(e){if(!e.game_name||!e.date_played||!Array.isArray(e.players)||e.players.length===0)throw Error(`Invalid format: missing game_name, date_played, or players`);let t=P(e),n=F(e),r=Date.now(),i;return await S.transaction(`rw`,[S.players,S.games,S.gameNights,S.matches,S.scoreEntries],async()=>{let a=[],o=await S.players.toArray(),s=new Map(o.map(e=>[e.displayName.trim().toLowerCase(),e.id])),c=new Map;for(let t of e.players){let e=t.name.trim().toLowerCase();if(s.has(e))c.set(e,s.get(e));else{let n=(o.length+a.length)%N.length,i=await S.players.add({displayName:t.name.trim(),color:N[n],active:!0,createdAt:r});c.set(e,i),a.push(t.name.trim())}}let l=await S.games.toArray(),u=e.game_name.trim().toLowerCase(),d=l.find(e=>e.name.trim().toLowerCase()===u),f;if(d)f=d.id;else{let i={name:e.game_name.trim(),scoringMode:t,createdAt:r};n&&(i.roundLabels=n),f=await S.games.add(i)}let p=await S.gameNights.add({title:e.game_name.trim(),date:e.date_played,createdAt:r}),m=e.players.map(e=>c.get(e.name.trim().toLowerCase())),h=I(e,c,t),g=await S.matches.add({gameNightId:p,gameId:f,playerIds:m,status:e.is_complete?`completed`:`active`,winnerId:h,createdAt:r}),_=[];for(let n of e.players){let e=c.get(n.name.trim().toLowerCase());for(let i of n.round_scores){let n={matchId:g,playerId:e,roundNumber:i.round_number,value:i.round_score,createdAt:r};t===`phase10`&&i.phase!==void 0&&(n.note=JSON.stringify({phase:i.phase,completed:!!i.phase_completed})),_.push(n)}}await S.scoreEntries.bulkAdd(_);let v=Math.max(...e.players.flatMap(e=>e.round_scores.map(e=>e.round_number)));i={gameName:e.game_name.trim(),date:e.date_played,playerCount:e.players.length,newPlayers:a,roundCount:v}}),window.dispatchEvent(new CustomEvent(`scorekeeper:datachanged`)),i}var R=`scorekeeper_sync_config`;function z(){try{let e=localStorage.getItem(R);if(!e)return null;let t=JSON.parse(e);return t.provider||=`github`,t}catch{return null}}function B(e){localStorage.setItem(R,JSON.stringify(e))}function V(e){return(e.baseUrl??``).replace(/\/+$/,``)}function H(e){return e.provider===`gitea`?`${V(e)}/api/v1/repos/${e.username}/${e.repo}/contents/${e.filePath}`:`https://api.github.com/repos/${e.username}/${e.repo}/contents/${e.filePath}`}function U(e){return e.provider===`gitea`?`${V(e)}/api/v1/repos/${e.username}/${e.repo}`:`https://api.github.com/repos/${e.username}/${e.repo}`}function W(e){let t={Authorization:`token ${e.pat}`,"Content-Type":`application/json`};return e.provider===`github`&&(t.Accept=`application/vnd.github.v3+json`),t}function G(e){return e.provider===`gitea`?`Gitea`:`GitHub`}function K(e,t){let n=t===`gitea`?`Gitea`:`GitHub`;switch(e){case 401:return`Unauthorized – check your ${t===`gitea`?`API key`:`Personal Access Token`}`;case 403:return`Forbidden – token may lack repo write permissions`;case 404:return`Not Found – check username, repo name, and file path`;case 409:return`Conflict – file was modified remotely; try syncing from ${n} first`;case 422:return`Unprocessable – invalid request data`;case 429:return`Rate Limited – too many requests, try again in a minute`;default:return`${n} API error (HTTP ${e})`}}function q(e){return e.username?e.repo?e.pat?e.provider===`gitea`&&!e.baseUrl?`Base URL is required for Gitea`:null:e.provider===`gitea`?`API key is required`:`Personal Access Token is required`:`Repository name is required`:`Username is required`}async function J(e){try{let t=await fetch(U(e),{headers:W(e)});if(t.ok){let n=await t.json();return{ok:!0,message:`Connected to ${n.full_name??`${e.username}/${n.name??e.repo}`} on ${G(e)}`}}return{ok:!1,message:K(t.status,e.provider)}}catch(e){return{ok:!1,message:`Network error: ${e instanceof Error?e.message:String(e)}`}}}async function Y(e){try{let t=await x(),n=JSON.stringify(t,null,2),r=btoa(unescape(encodeURIComponent(n))),i=H(e),a=W(e),o,s=await fetch(`${i}?ref=${encodeURIComponent(e.branch)}`,{headers:a});if(s.ok)o=(await s.json()).sha;else if(s.status!==404)return{ok:!1,message:K(s.status,e.provider)};let c={message:`Scorekeeper sync ${new Date().toISOString()}`,content:r,branch:e.branch};o&&(c.sha=o);let l=await fetch(i,{method:`PUT`,headers:a,body:JSON.stringify(c)});return l.ok?(B({...e,lastSync:Date.now()}),{ok:!0,message:`Synced to ${G(e)} successfully`}):{ok:!1,message:K(l.status,e.provider)}}catch(e){return{ok:!1,message:`Sync error: ${e instanceof Error?e.message:String(e)}`}}}async function X(e,t){try{let n=`${H(e)}?ref=${encodeURIComponent(e.branch)}`,i=await fetch(n,{headers:W(e)});if(!i.ok)return{ok:!1,message:K(i.status,e.provider)};let a=await i.json(),o=decodeURIComponent(escape(atob(a.content.replace(/\n/g,``)))),s=JSON.parse(o),c=await x();return(c.players.length>0||c.gameNights.length>0||c.matches.length>0)&&!await t()?{ok:!1,message:`Sync cancelled`}:(await r(s),B({...e,lastSync:Date.now()}),{ok:!0,message:`Synced from ${G(e)} successfully`})}catch(e){return{ok:!1,message:`Sync error: ${e instanceof Error?e.message:String(e)}`}}}var Z=class{players=[];games=[];syncConfig=null;roomConfig=null;editingPlayerId=null;editingGameId=null;activeTab=`players`;async load(){let[e,t]=await Promise.all([f(),i()]);this.players=e,this.games=t,this.syncConfig=z(),this.roomConfig=d()}formatTimestamp(e){return e?new Date(e).toLocaleString():`Never`}render(){let e=(document.documentElement.getAttribute(`data-theme`)??`dark`)!==`light`,t=this.activeTab,n=e=>e===t?``:`style="display:none"`;return`
            ...
            <div class="card">
              <div class="text-sm" style="display:flex;flex-direction:column;gap:0.4rem">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="font-semibold">59</span>
                </div>
                <div class="flex items-center gap-2">
                  <span class="text-muted">Built</span>
                  <span class="font-semibold">${new Date(`2026-05-30T16:21:12.587Z`).toLocaleString()}</span>
                </div>
              </div>
            </div>
          </section>
    ...
    `).join(``)}renderPlayerForm(e){let t=e?.displayName??``,n=e?.color??N[0],r=e?.active??!0,i=N.map(e=>`
