import{O as e,_ as t,f as n,g as r,j as i,p as a}from"./index-Bchd81E0.js";import{n as o}from"./utils-BZltm_RL.js";var s=class{players=[];games=[];matches=[];pending=null;async load(){let[t,n]=await Promise.all([i(),e()]);this.players=t.filter(e=>e.active),this.games=n}render(){let e=new Date().toISOString().split(`T`)[0],t=this.games.length===0?`<option value="">No games — add in Settings</option>`:this.games.map(e=>`<option value="${e.id}">${o(e.name)}</option>`).join(``),n=this.players.length===0?`<p class="text-sm text-muted">No players — add in Settings</p>`:this.players.map(e=>`
          <label class="checkbox-item" for="player-check-${e.id}">
            <input type="checkbox" id="player-check-${e.id}" value="${e.id}" class="match-player-check" aria-label="${o(e.displayName)}">
            <span class="player-dot" style="background:${e.color}"></span>
            <span>${o(e.displayName)}</span>
          </label>
    ...
    `}renderMatchList(){return this.matches.length===0?`<p class="text-sm text-muted" id="no-matches-msg">No matches added yet. Add at least one above.</p>`:this.matches.map((e,t)=>{let n=e.playerNames[e.firstDealerIndex]??``;return`
        <div class="player-list-item" data-match-index="${t}">
          ...
          <div style="flex:1; min-width:0">
            <div class="font-semibold text-sm">${o(e.gameName)}</div>
            <div class="text-xs text-muted">${e.playerNames.map(e=>o(e)).join(` → `)}</div>
            ${n?`<div class="text-xs text-muted">🃏 First dealer: ${o(n)}</div>`:``}
          </div>
          <div class="actions">
      ...
      `}).join(``)}renderConfigurePlayerList(){return this.pending?this.pending.playerIds.map((e,t)=>{let n=this.players.find(t=>t.id===e);if(!n)return``;let r=t===this.pending.firstDealerIndex,i=t===0,a=t===this.pending.playerIds.length-1;return`
        <div class="configure-player-row">
          <div class="configure-player-info">
            ...
            <span class="configure-player-name">${o(n.displayName)}</span>
          </div>
