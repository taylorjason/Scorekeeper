import{O as e,_ as t,f as n,g as r,j as i,p as a}from"./index-CONtZ6Kl.js";import{n as o}from"./utils-BZltm_RL.js";var s=class{players=[];games=[];matches=[];pending=null;async load(){let[t,n]=await Promise.all([i(),e()]);this.players=t.filter(e=>e.active),this.games=n}render(){let e=new Date().toISOString().split(`T`)[0],t=this.games.length===0?`<option value="">No games — add in Settings</option>`:this.games.map(e=>`<option value="${e.id}">${o(e.name)}</option>`).join(``),n=this.players.length===0?`<p class="text-sm text-muted">No players — add in Settings</p>`:this.players.map(e=>`
