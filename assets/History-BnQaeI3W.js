import{A as e,D as t,M as n,_ as r,f as i,p as a,x as o,y as s}from"./index-tA6yxZO2.js";import{n as c}from"./utils-BZltm_RL.js";var l=class{nights=[];allPlayers=[];allGames=[];filterPlayerId=null;filterGameId=null;filterDateFrom=``;sortOrder=`desc`;async load(){let[r,i,a]=await Promise.all([t(),s.players.toArray(),s.games.toArray()]);this.allPlayers=i,this.allGames=a,this.nights=await Promise.all(r.map(async t=>{let r=await e(t.id);return{night:t,matches:await Promise.all(r.map(async e=>{let t=a.find(t=>t.id===e.gameId),r=e.playerIds.map(e=>i.find(t=>t.id===e)).filter(e=>e!==void 0),o=e.winnerId===void 0?void 0:i.find(t=>t.id===e.winnerId),s=await n(e.id),c=r.map(e=>({player:e,total:s.filter(t=>t.playerId===e.id).reduce((e,t)=>e+t.value,0)}));return t?.scoringMode===`low`?c.sort((e,t)=>e.total-t.total):c.sort((e,t)=>t.total-e.total),{match:e,game:t,players:r,winner:o,playerTotals:c,entries:s}}))}}))}formatDate(e){return new Date(e+`T00:00:00`).toLocaleDateString(`en-US`,{weekday:`short`,month:`short`,day:`numeric`,year:`numeric`})}getFilteredNights(){let e=this.nights;if(this.filterPlayerId!==null){let t=this.filterPlayerId;e=e.filter(e=>e.matches.some(e=>e.match.playerIds.includes(t)))}if(this.filterGameId!==null){let t=this.filterGameId;e=e.filter(e=>e.matches.some(e=>e.match.gameId===t))}return this.filterDateFrom&&(e=e.filter(e=>e.night.date>=this.filterDateFrom)),this.sortOrder===`asc`&&(e=[...e].reverse()),e}dateFromPreset(e){if(!e)return``;let t=new Date;switch(e){case`30d`:t.setDate(t.getDate()-30);break;case`3mo`:t.setMonth(t.getMonth()-3);break;case`6mo`:t.setMonth(t.getMonth()-6);break;case`1yr`:t.setFullYear(t.getFullYear()-1);break;default:return``}return t.toISOString().slice(0,10)}render(){let e=this.getFilteredNights(),t=[`<button class="tab-btn ${this.filterPlayerId===null?`active`:``}" data-player-filter="null">All Players</button>`,...this.allPlayers.map(e=>`<button class="tab-btn ${this.filterPlayerId===e.id?`active`:``}" data-player-filter="${e.id}">
      ...
      <main class="view" aria-label="History">
        ...
        <section class="mb-4" aria-label="Filters">
          ...
          <div class="filter-bar" role="group" aria-label="Date range filter">
            ${[[``,`All Time`],[`30d`,`Last 30d`],[`3mo`,`Last 3mo`],[`6mo`,`Last 6mo`],[`1yr`,`This Year`]].map(([e,t])=>`<button class="tab-btn ${this.filterDateFrom===this.dateFromPreset(e)?`active`:``}" data-date-preset="${e}">${t}</button>`).join(``)}
          </div>
    ...
    `}showScoreTableModal(e){let t=this.nights.flatMap(e=>e.matches).find(t=>t.match.id===e);if(!t)return;let n=[...new Set(t.entries.map(e=>e.roundNumber))].sort((e,t)=>e-t),r=t.game?.roundLabels,i=n.map(e=>`<th>${r?.[e-1]?c(r[e-1]):`R${e}`}</th>`).join(``),a=e=>{if(e===`first_out`)return!0;if(e)try{return!!JSON.parse(e).firstOut}catch{}return!1},o=t.playerTotals.map(({player:e,total:r},i)=>{let o=n.map(n=>{let r=t.entries.find(t=>t.playerId===e.id&&t.roundNumber===n);return`<td class="score-table-score">${r!==void 0&&a(r.note)?`⚡ `:``}${r===void 0?`—`:r.value}</td>`}).join(``);return`
        ...
        </tr>`}).join(``),s=document.createElement(`div`);s.id=`score-table-modal`,s.setAttribute(`role`,`dialog`),s.setAttribute(`aria-modal`,`true`),s.setAttribute(`aria-label`,`Score Table`),s.innerHTML=`
    ...
    `,document.body.appendChild(s),s.querySelectorAll(`.modal-close-btn, .modal-backdrop`).forEach(e=>{e.addEventListener(`click`,()=>s.remove())})}showAddMatchModal(e){let t=document.getElementById(`add-match-modal`);t&&t.remove();let n=this.allGames.map(e=>`<option value="${e.id}">${c(e.name)}</option>`).join(``),o=this.allPlayers.map(e=>`
