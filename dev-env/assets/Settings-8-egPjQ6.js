import{C as e,I as t,N as n,O as r,P as i,a,b as o,c as s,d as c,f as l,h as u,i as d,j as f,l as p,n as m,o as h,r as g,s as _,t as v,u as y,v as b,w as x,y as S}from"./index-B_MB3V81.js";var C=Object.create,w=Object.defineProperty,T=Object.getOwnPropertyDescriptor,E=Object.getOwnPropertyNames,D=Object.getPrototypeOf,O=Object.prototype.hasOwnProperty,k=(e,t)=>()=>(t||(e((t={exports:{}}).exports,t),e=null),t.exports),A=(e,t,n,r)=>{if(t&&typeof t==`object`||typeof t==`function`)for(var i=E(t),a=0,o=i.length,s;a<o;a++)s=i[a],!O.call(e,s)&&s!==n&&w(e,s,{get:(e=>t[e]).bind(null,s),enumerable:!(r=T(t,s))||r.enumerable});return e},j=(e,t,n)=>(n=e==null?{}:C(D(e)),A(t||!e||!e.__esModule?w(n,`default`,{value:e,enumerable:!0}):n,e)),M=[`#ef4444`,`#f97316`,`#f59e0b`,`#eab308`,`#10b981`,`#14b8a6`,`#3b82f6`,`#6366f1`,`#8b5cf6`,`#ec4899`,`#64748b`,`#d97706`];function N(e){return e.players.some(e=>e.round_scores.some(e=>e.phase!==void 0))?`phase10`:`rounds`}function P(e){let t=new Map;for(let n of e.players)for(let e of n.round_scores)t.has(e.round_number)||t.set(e.round_number,e.round_name);let n=[...t.entries()].sort((e,t)=>e[0]-t[0]);if(!n.every(([e,t])=>t===String(e)))return n.map(([,e])=>e)}function F(e,t,n){let r=e.players.filter(e=>e.is_winner);if(r.length===0)return;if(r.length===1)return t.get(r[0].name.trim().toLowerCase());let i=n===`phase10`||n===`low`?r.reduce((e,t)=>e.final_score<t.final_score?e:t):r.reduce((e,t)=>e.final_score>t.final_score?e:t);return t.get(i.name.trim().toLowerCase())}async function I(e){if(!e.game_name||!e.date_played||!Array.isArray(e.players)||e.players.length===0)throw Error(`Invalid format: missing game_name, date_played, or players`);let t=N(e),n=P(e),r=Date.now(),i;return await S.transaction(`rw`,[S.players,S.games,S.gameNights,S.matches,S.scoreEntries],async()=>{let a=[],o=await S.players.toArray(),s=new Map(o.map(e=>[e.displayName.trim().toLowerCase(),e.id])),c=new Map;for(let t of e.players){let e=t.name.trim().toLowerCase();if(s.has(e))c.set(e,s.get(e));else{let n=(o.length+a.length)%M.length,i=await S.players.add({displayName:t.name.trim(),color:M[n],active:!0,createdAt:r});c.set(e,i),a.push(t.name.trim())}}let l=await S.games.toArray(),u=e.game_name.trim().toLowerCase(),d=l.find(e=>e.name.trim().toLowerCase()===u),f;if(d)f=d.id;else{let i={name:e.game_name.trim(),scoringMode:t,createdAt:r};n&&(i.roundLabels=n),f=await S.games.add(i)}let p=await S.gameNights.add({title:e.game_name.trim(),date:e.date_played,createdAt:r}),m=e.players.map(e=>c.get(e.name.trim().toLowerCase())),h=F(e,c,t),g=await S.matches.add({gameNightId:p,gameId:f,playerIds:m,status:e.is_complete?`completed`:`active`,winnerId:h,createdAt:r}),_=[];for(let n of e.players){let e=c.get(n.name.trim().toLowerCase());for(let i of n.round_scores){let n={matchId:g,playerId:e,roundNumber:i.round_number,value:i.round_score,createdAt:r};t===`phase10`&&i.phase!==void 0&&(n.note=JSON.stringify({phase:i.phase,completed:!!i.phase_completed})),_.push(n)}}await S.scoreEntries.bulkAdd(_);let v=Math.max(...e.players.flatMap(e=>e.round_scores.map(e=>e.round_number)));i={gameName:e.game_name.trim(),date:e.date_played,playerCount:e.players.length,newPlayers:a,roundCount:v}}),window.dispatchEvent(new CustomEvent(`scorekeeper:datachanged`)),i}var L=`scorekeeper_sync_config`;function R(){try{let e=localStorage.getItem(L);if(!e)return null;let t=JSON.parse(e);return t.provider||=`github`,t}catch{return null}}function z(e){localStorage.setItem(L,JSON.stringify(e))}function B(e){return(e.baseUrl??``).replace(/\/+$/,``)}function V(e){return e.provider===`gitea`?`${B(e)}/api/v1/repos/${e.username}/${e.repo}/contents/${e.filePath}`:`https://api.github.com/repos/${e.username}/${e.repo}/contents/${e.filePath}`}function H(e){return e.provider===`gitea`?`${B(e)}/api/v1/repos/${e.username}/${e.repo}`:`https://api.github.com/repos/${e.username}/${e.repo}`}function U(e){let t={Authorization:`token ${e.pat}`,"Content-Type":`application/json`};return e.provider===`github`&&(t.Accept=`application/vnd.github.v3+json`),t}function W(e){return e.provider===`gitea`?`Gitea`:`GitHub`}function G(e,t){let n=t===`gitea`?`Gitea`:`GitHub`;switch(e){case 401:return`Unauthorized – check your ${t===`gitea`?`API key`:`Personal Access Token`}`;case 403:return`Forbidden – token may lack repo write permissions`;case 404:return`Not Found – check username, repo name, and file path`;case 409:return`Conflict – file was modified remotely; try syncing from ${n} first`;case 422:return`Unprocessable – invalid request data`;case 429:return`Rate Limited – too many requests, try again in a minute`;default:return`${n} API error (HTTP ${e})`}}function K(e){return e.username?e.repo?e.pat?e.provider===`gitea`&&!e.baseUrl?`Base URL is required for Gitea`:null:e.provider===`gitea`?`API key is required`:`Personal Access Token is required`:`Repository name is required`:`Username is required`}async function q(e){try{let t=await fetch(H(e),{headers:U(e)});if(t.ok){let n=await t.json();return{ok:!0,message:`Connected to ${n.full_name??`${e.username}/${n.name??e.repo}`} on ${W(e)}`}}return{ok:!1,message:G(t.status,e.provider)}}catch(e){return{ok:!1,message:`Network error: ${e instanceof Error?e.message:String(e)}`}}}async function J(e){try{let t=await x(),n=JSON.stringify(t,null,2),r=btoa(unescape(encodeURIComponent(n))),i=V(e),a=U(e),o,s=await fetch(`${i}?ref=${encodeURIComponent(e.branch)}`,{headers:a});if(s.ok)o=(await s.json()).sha;else if(s.status!==404)return{ok:!1,message:G(s.status,e.provider)};let c={message:`Scorekeeper sync ${new Date().toISOString()}`,content:r,branch:e.branch};o&&(c.sha=o);let l=await fetch(i,{method:`PUT`,headers:a,body:JSON.stringify(c)});return l.ok?(z({...e,lastSync:Date.now()}),{ok:!0,message:`Synced to ${W(e)} successfully`}):{ok:!1,message:G(l.status,e.provider)}}catch(e){return{ok:!1,message:`Sync error: ${e instanceof Error?e.message:String(e)}`}}}async function Y(e,t){try{let r=`${V(e)}?ref=${encodeURIComponent(e.branch)}`,i=await fetch(r,{headers:U(e)});if(!i.ok)return{ok:!1,message:G(i.status,e.provider)};let a=await i.json(),o=decodeURIComponent(escape(atob(a.content.replace(/\n/g,``)))),s=JSON.parse(o),c=await x();return(c.players.length>0||c.gameNights.length>0||c.matches.length>0)&&!await t()?{ok:!1,message:`Sync cancelled`}:(await n(s),z({...e,lastSync:Date.now()}),{ok:!0,message:`Synced from ${W(e)} successfully`})}catch(e){return{ok:!1,message:`Sync error: ${e instanceof Error?e.message:String(e)}`}}}var X=[`#ef4444`,`#f97316`,`#f59e0b`,`#eab308`,`#10b981`,`#14b8a6`,`#3b82f6`,`#6366f1`,`#8b5cf6`,`#ec4899`,`#64748b`,`#d97706`],Z=class{players=[];games=[];syncConfig=null;roomConfig=null;editingPlayerId=null;editingGameId=null;async load(){let[e,t]=await Promise.all([f(),r()]);this.players=e,this.games=t,this.syncConfig=R(),this.roomConfig=d()}escHtml(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}formatTimestamp(e){return e?new Date(e).toLocaleString():`Never`}render(){let e=(document.documentElement.getAttribute(`data-theme`)??`dark`)!==`light`;return`
    ...
    `).join(``)}renderPlayerForm(e){let t=e?.displayName??``,n=e?.color??X[0],r=e?.active??!0,i=X.map(e=>`
