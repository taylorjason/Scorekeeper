        ...
        <div class="card card-clickable mb-3 night-card" data-night-id="${e.id}" role="button" aria-label="Open ${e.title}">
          <div class="flex items-center gap-3">
            <div>
              ...
              <div class="font-semibold">${this.escHtml(e.title)}</div>
            </div>
            <span style="margin-left:auto; color:var(--text-muted)">›</span>
          </div>
        </div>
      `).join(``);let r=``;return r=this.leaderboard.length===0?`<div class="text-sm text-muted" style="padding: 0.5rem 0">No stats yet — play some games!</div>`:`
              ...
              <tr>
                ...
                <td>
                  <div class="flex items-center gap-2">
                    ...
                    ${this.escHtml(e.player.displayName)}
                  </div>
                </td>
