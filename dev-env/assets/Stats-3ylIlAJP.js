import{d as e,f as t,y as n}from"./index-iE8rVY-G.js";import{n as r,r as i,t as a}from"./utils-BZltm_RL.js";import{n as o,r as s,t as c}from"./stats-CqcXjDC2.js";async function l(){let[e,t,r,i]=await Promise.all([n.matches.where(`status`).equals(`completed`).toArray(),n.players.toArray(),n.games.toArray(),n.gameNights.toArray()]),o=new Map(t.map(e=>[e.id,e])),s=new Map(r.map(e=>[e.id,e])),c=new Map(i.map(e=>[e.id,e])),l=e.map(e=>e.id),u=l.length>0?await n.scoreEntries.where(`matchId`).anyOf(l).toArray():[],d=new Map;for(let e of u){let t=d.get(e.matchId)??[];t.push(e),d.set(e.matchId,t)}let f=[];for(let t of e){let e=s.get(t.gameId),n=c.get(t.gameNightId);if(!e||!n)continue;let r=d.get(t.id)??[],i=a(r,t.createdAt);for(let a of r){let r=o.get(a.playerId);if(!r)continue;let s=a.note===`first_out`;if(!s&&a.note)try{JSON.parse(a.note).firstOut&&(s=!0)}catch{}f.push({entryId:a.id,playerId:a.playerId,playerName:r.displayName,playerColor:r.color,matchId:t.id,gameId:t.gameId,gameName:e.name,nightId:t.gameNightId,nightDate:n.date,roundNumber:a.roundNumber,value:a.value,note:a.note,entryCreatedAt:a.createdAt,isFirstOut:s,isWinner:t.winnerId===a.playerId,roundDuration:i.get(a.roundNumber)})}}return f}function u(e,t){let n=e;if(t.filters.playerIds.length>0){let e=new Set(t.filters.playerIds);n=n.filter(t=>e.has(t.playerId))}if(t.filters.gameIds.length>0){let e=new Set(t.filters.gameIds);n=n.filter(t=>e.has(t.gameId))}if(t.filters.dateFrom&&(n=n.filter(e=>e.nightDate>=t.filters.dateFrom)),t.field===`roundDuration`){let e=new Set;n=n.filter(t=>{let n=`${t.matchId}:${t.roundNumber}`;return e.has(n)?!1:(e.add(n),!0)})}if(t.field===`isWin`){let e=new Set;n=n.filter(t=>{let n=`${t.matchId}:${t.playerId}`;return e.has(n)?!1:(e.add(n),!0)})}let r=new Map,i=e=>{switch(t.groupBy){case`player`:return{key:String(e.playerId),label:e.playerName,color:e.playerColor};case`game`:return{key:String(e.gameId),label:e.gameName};case`month`:return{key:e.nightDate.slice(0,7),label:e.nightDate.slice(0,7)};default:return{key:`all`,label:`All`}}};for(let e of n){let n;switch(t.field){case`value`:n=e.value;break;case`roundDuration`:n=e.roundDuration;break;case`isFirstOut`:n=e.isFirstOut?1:void 0;break;case`isWin`:n=e.isWinner?1:void 0;break}if(n===void 0)continue;let{key:a,label:o,color:s}=i(e),c=r.get(a)??{label:o,color:s,values:[]};c.values.push(n),r.set(a,c)}let a=(e,t)=>{if(e.length===0)return 0;switch(t){case`count`:return e.length;case`sum`:return e.reduce((e,t)=>e+t,0);case`avg`:return Math.round(e.reduce((e,t)=>e+t,0)/e.length);case`min`:return Math.min(...e);case`max`:return Math.max(...e)}},o=[];for(let[,e]of r.entries())o.push({label:e.label,value:a(e.values,t.metric),color:e.color,sampleSize:e.values.length});let s=t.metric===`min`||t.field===`roundDuration`;return o.sort((e,t)=>s?e.value-t.value:t.value-e.value),o}function d(e){if(!e)return``;let t=new Date;switch(e){case`30d`:t.setDate(t.getDate()-30);break;case`3mo`:t.setMonth(t.getMonth()-3);break;case`6mo`:t.setMonth(t.getMonth()-6);break;case`1yr`:t.setFullYear(t.getFullYear()-1);break;default:return``}return t.toISOString().slice(0,10)}function f(e,t){let n={value:`Round Score`,roundDuration:`Round Duration`,isFirstOut:`First-Out Events`,isWin:`Wins`};return(e===`isFirstOut`||e===`isWin`)&&t===`count`?n[e]:`${{count:`Count of`,sum:`Total`,avg:`Average`,min:`Shortest`,max:`Longest`}[t]} ${n[e]}`}var p=`scorekeeper_custom_stats`,m=`scorekeeper_claude_api_key`;function h(){try{return JSON.parse(localStorage.getItem(p)??`[]`)}catch{return[]}}function g(e){localStorage.setItem(p,JSON.stringify(e))}function _(){return localStorage.getItem(m)??``}function v(e){e?localStorage.setItem(m,e):localStorage.removeItem(m)}var y=`You are a JavaScript code generator for a board game scorekeeper app.
...
- Deduplicate rows where needed (e.g. one row per matchId+playerId for win counts).`;async function b(e,t,n){let r=JSON.stringify(n.slice(0,5),null,2),i=await fetch(`https://api.anthropic.com/v1/messages`,{method:`POST`,headers:{"content-type":`application/json`,"x-api-key":t,"anthropic-version":`2023-06-01`,"anthropic-dangerous-direct-browser-calls":`true`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:1024,system:y,messages:[{role:`user`,content:`Stat to compute: "${e}"\n\nSample data (first 5 rows):\n${r}\n\nGenerate the JavaScript function body now.`}]})});if(!i.ok){let e=await i.text();throw Error(`API error ${i.status}: ${e}`)}return((await i.json()).content.find(e=>e.type===`text`)?.text??``).replace(/^```[a-z]*\n?/i,``).replace(/\n?```$/i,``).trim()}function x(e,t){try{let n=Function(`rows`,e)(t);if(typeof n==`number`)return{type:`number`,value:n};if(typeof n==`string`)return{type:`text`,value:n};if(Array.isArray(n)){let e=n.map(e=>{let t=e;return{label:String(t.label??``),value:Number(t.value??0),color:typeof t.color==`string`?t.color:void 0}});return e.sort((e,t)=>t.value-e.value),{type:`table`,rows:e}}return{type:`text`,value:JSON.stringify(n)}}catch(e){return{type:`error`,message:e instanceof Error?e.message:String(e)}}}var S=[{id:`first-out`,label:`⚡ First-Out Leaders`,query:{metric:`count`,field:`isFirstOut`,filters:{playerIds:[],gameIds:[],dateFrom:``},groupBy:`player`}},{id:`fastest`,label:`⏱ Fastest Rounds`,query:{metric:`min`,field:`roundDuration`,filters:{playerIds:[],gameIds:[],dateFrom:``},groupBy:`game`}},{id:`avg-score`,label:`📊 Avg Round Score`,query:{metric:`avg`,field:`value`,filters:{playerIds:[],gameIds:[],dateFrom:``},groupBy:`player`}},{id:`wins`,label:`🏆 Win Leaders`,query:{metric:`count`,field:`isWin`,filters:{playerIds:[],gameIds:[],dateFrom:``},groupBy:`player`}},{id:`monthly`,label:`📅 Last 3 Months`,query:{metric:`count`,field:`isWin`,filters:{playerIds:[],gameIds:[],dateFrom:d(`3mo`)},groupBy:`month`}}],C=class{players=[];games=[];selectedPlayerId=null;leaderboard=[];playerStats=null;activeMainTab=`overview`;factTable=[];currentQuery=S[0].query;queryResults=null;_winsChart=null;_trendChart=null;_explorerChart=null;customStats=[];customApiKey=``;_evaluatedStats=new Map;_customGenerating=!1;async load(){let[e,t,r,i]=await Promise.all([n.players.toArray(),n.games.toArray(),o(),l()]);this.players=e.filter(e=>e.active),this.games=t,this.leaderboard=r,this.factTable=i,this.selectedPlayerId===null&&this.players.length>0&&(this.selectedPlayerId=this.players[0].id),this.selectedPlayerId!==null&&(this.playerStats=await s(this.selectedPlayerId)),this.customStats=h(),this.customApiKey=_(),this._evaluatedStats.clear();for(let e of this.customStats)this._evaluatedStats.set(e.id,x(e.code,this.factTable))}render(){let e=this._renderOverview();return`
    ...
    `:``;return`
      ...
      ${a}
    `}_renderExplorerTab(){let e=this.currentQuery,t=S.map(e=>`<button class="quick-insight-btn" data-insight="${e.id}">${e.label}</button>`).join(``),n=`<option value="">All Players</option>`+this.players.map(t=>`<option value="${t.id}" ${e.filters.playerIds[0]===t.id?`selected`:``}>${r(t.displayName)}</option>`).join(``),i=`<option value="">All Games</option>`+this.games.map(t=>`<option value="${t.id}" ${e.filters.gameIds[0]===t.id?`selected`:``}>${r(t.name)}</option>`).join(``),a=[{value:``,label:`All time`},{value:`30d`,label:`Last 30 days`},{value:`3mo`,label:`Last 3 months`},{value:`6mo`,label:`Last 6 months`},{value:`1yr`,label:`This year`}],o=a.find(t=>t.value!==``&&d(t.value)===e.filters.dateFrom)?.value??``,s=a.map(e=>`<option value="${e.value}" ${o===e.value?`selected`:``}>${e.label}</option>`).join(``),c=[{value:`value`,label:`Round Score`},{value:`roundDuration`,label:`Round Duration`},{value:`isFirstOut`,label:`First-Out Events`},{value:`isWin`,label:`Wins`}].map(t=>`<option value="${t.value}" ${e.field===t.value?`selected`:``}>${t.label}</option>`).join(``);return`
      <div class="explorer-wrap">
        ...
        <section class="query-builder card mb-4" aria-label="Custom query builder">
          ...
          <div class="qb-row">
            ...
            <select class="form-select qb-select" id="qb-player">${n}</select>
          </div>
          <div class="qb-row">
    ...
    `}_bindCustomTab(){document.getElementById(`custom-stat-save-key`)?.addEventListener(`click`,()=>{let e=document.getElementById(`custom-stat-api-key`)?.value.trim()??``;e&&(v(e),this.customApiKey=e,this.reRender())}),document.getElementById(`custom-stat-clear-key`)?.addEventListener(`click`,()=>{v(``),this.customApiKey=``,this.reRender()}),document.getElementById(`custom-stat-generate`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`custom-stat-desc`)?.value.trim()??``;if(!e){t(`Describe a stat first`,`error`);return}if(!this.customApiKey){t(`Add your Claude API key first`,`error`);return}this._customGenerating=!0,this.reRender();try{let n=await b(e,this.customApiKey,this.factTable),r={id:Date.now().toString(),name:e.length>60?e.slice(0,57)+`…`:e,description:e,code:n,createdAt:Date.now()};this._evaluatedStats.set(r.id,x(n,this.factTable)),this.customStats=[r,...this.customStats],g(this.customStats),t(`Stat added!`,`success`)}catch(e){t(e instanceof Error?e.message:`Failed to generate stat`,`error`)}finally{this._customGenerating=!1,this.reRender()}}),document.querySelectorAll(`.custom-stat-delete`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.statId;t&&(this.customStats=this.customStats.filter(e=>e.id!==t),this._evaluatedStats.delete(t),g(this.customStats),this.reRender())})})}reRender(){this.teardown();let e=document.getElementById(`view-container`);e&&(e.innerHTML=this.render(),this.afterRender())}};export{C as Stats};